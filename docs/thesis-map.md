...
| `va-frontend`  | T1     | lexer, parser, AST, elaboration → `va-ir`         | `va-ir`                  | TBD   | 🟢 lex/parse/elaborate | A rigorous Verilog-A subset grammar + parser study — see `token-reference.md` for the token-by-token reference this thesis is built on. |
