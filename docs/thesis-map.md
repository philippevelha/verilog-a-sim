...
| `va-codegen`   | T2      | IR → automatic differentiation → model instances  | `va-ir`, `va-abi`        | TBD   | 🟢 AD + lowering (incl. local variables, `if`/`else`, `case`, loops, potential contributions incl. mixed flow/potential branches) + charge; 37/115 real corpus files simulatable (see `roadmap.md` T2.2) | An AD-for-compact-models report (forward vs reverse, FD validation). |
