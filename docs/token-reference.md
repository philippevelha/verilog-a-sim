...
| `laplace_nd` | **Implemented in AC** (Tier C, 2026-08-07): `H(jω)` is evaluated per frequency point and stamped as `G = Re(H)`, `C = Im(H)/ω`, which the assembled `G + jω·C` reconstitutes exactly. **Still folds to its DC gain in DC and transient** — a time-domain Laplace filter is a convolution, a separate problem (`docs/proposals/frequency-domain.md` §2). Parses as a call (`laplace_nd(in, num[, den])`); elaboration has no builtin → `unknown function` | Laplace-domain transfer-function filter, numerator/denominator coefficient form | `H(jω)` per frequency point in AC; DC gain in DC/transient | Analog-block only, signal-flow filter | No C analogue (a continuous-time transfer function) |
| `laplace_np` | **Implemented in AC** (Tier C, 2026-08-07): `H(jω)` is evaluated per frequency point and stamped as `G = Re(H)`, `C = Im(H)/ω`, which the assembled `G + jω·C` reconstitutes exactly. **Still folds to its DC gain in DC and transient** — a time-domain Laplace filter is a convolution, a separate problem (`docs/proposals/frequency-domain.md` §2). Same family as `laplace_nd`, pole/zero form | Laplace-domain filter, pole/zero form | `H(jω)` per frequency point in AC; DC gain in DC/transient | Analog-block only | No C analogue |
| `laplace_zd` | **Implemented in AC** (Tier C, 2026-08-07): `H(jω)` is evaluated per frequency point and stamped as `G = Re(H)`, `C = Im(H)/ω`, which the assembled `G + jω·C` reconstitutes exactly. **Still folds to its DC gain in DC and transient** — a time-domain Laplace filter is a convolution, a separate problem (`docs/proposals/frequency-domain.md` §2). Same family, Z-domain numerator/denominator form | Z-domain (discrete) filter | `H(jω)` per frequency point in AC; DC gain in DC/transient | Analog-block only | Closest: a digital IIR filter's difference equation, but expressed declaratively |
| `laplace_zp` | **Implemented in AC** (Tier C, 2026-08-07): `H(jω)` is evaluated per frequency point and stamped as `G = Re(H)`, `C = Im(H)/ω`, which the assembled `G + jω·C` reconstitutes exactly. **Still folds to its DC gain in DC and transient** — a time-domain Laplace filter is a convolution, a separate problem (`docs/proposals/frequency-domain.md` §2). Same family, Z-domain pole/zero form | Z-domain (discrete) filter | `H(jω)` per frequency point in AC; DC gain in DC/transient | Analog-block only | Same as `laplace_zd` |
...
| `last_crossing` | Parses as a call (`last_crossing(expr, dir)`); elaboration has no builtin → `unknown function` | Returns the simulation time of the last zero-crossing of `expr` | `H(jω)` per frequency point in AC; DC gain in DC/transient | Analog-block only | No C analogue |
