...
| `cross` | Parses as a call (`cross(expr, dir[, ...])`) if written bare; its one real usage, `@(cross(...))`, is **refused** (2026-09-06) — it used to be discarded wholesale by `skip_balanced_parens`, which left the body running at every solve point. A monitored event never fires in a static solve either, so discarding it was wrong in every analysis, not just transient | Zero-crossing event detector (LRM §5.7ish) | Neither path currently evaluates its arguments | Analog-block only (event control) | No C analogue (continuous zero-crossing detection needs the solver's own state) |
