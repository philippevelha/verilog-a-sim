...
- **Expressions and Evaluation**: A recognised bare trigger is parsed properly — `cross`/`above`/
  ...
  become a zero-argument `Builtin` reading the analysis context, and an `or` list of *those two*
  composes into a `||` chain (2026-09-07, v0.9.9) — sound because both are answerable at any
  evaluation rather than scheduled. Anything else — every other compound trigger, the
  analysis-filter form `initial_step("dc")`, and every event this engine cannot schedule — is
  **refused**, not discarded: running a body unconditionally is a wrong answer rather than a
  missing feature. The two remaining discarded triggers are named above.
...
| `absdelay` | **Implemented in AC as an exact delay** (§6 Interface α change ratified 2026-09-01, `docs/proposals/absdelay.md`): lowered to `Builtin::Absdelay` rather than folded away at elaboration, and stamped as `H(jw) = exp(-jw*tau)` through the same `G = Re(H)`, `C = Im(H)/w` route the `laplace_*` family uses — exact, not a rational approximation, since the response repeats every `1/tau` in frequency and no finite-order filter does that. **Still folds to its undelayed input in transient**, where a real delay needs an interpolated history buffer (stage 2); that fold is now *reported* rather than silent — `va-cli` warns on a transient run and `check` counts such files apart from plain passes. Reserved since 2026-08-31, having had a row in this table while being absent from `RESERVED_WORDS` | `absdelay(value, delay[, max_delay])` call; arguments normalized to `[value, delay]` at elaboration | `exp(-jw*tau)` in AC; the identity at DC (`H(0) = 1`, the correct steady state) and, for now, in transient. `max_delay` is parsed and dropped — only a time-domain implementation needs it, to size a buffer | Analog-block only; must be a bare top-level additive term of a contribution (its gain is complex) | No C analogue |
...
| `above` | `@(above(expr[, time_tol[, expr_tol[, enable]]]))` is **implemented** (2026-09-06, v0.9.7). *Not* a one-sided `cross`: per LRM §5.10.2 it "also triggers during initialization or dc", which is the whole point — a signal already past the threshold never crosses it, so a `cross` on it never fires. Lowers to a rising `EventSite::Cross` with `at_initialization`. Note the argument list is shifted against `cross`'s: position 2 is a *tolerance*, there being no direction argument | Threshold detector that also fires when already above | Fires in DC/initialization *and* on a rising crossing in transient | Analog-block only (event control) | No C analogue |
| `absdelta` | Reserved (added to `RESERVED_WORDS` 2026-09-06 — LRM Table B.1 has it and this lexer did not), and **refused** in a trigger: the last monitored event still unimplemented (LRM §5.10.4) | Fires on a change in `expr` exceeding `delta` | N/A | Analog-block only (event control) | No C analogue |
| `cross` | Parses as a call (`cross(expr, dir[, ...])`) if written bare; its real usage, `@(cross(expr, dir))`, is **implemented** (2026-09-06, v0.9.3): elaboration registers a `va_ir::CrossSite` and desugars the body to `if (CrossFired(k)) …`, codegen reports the monitored expression on Interface β's event channel, and the integrator detects the crossing and re-solves that timepoint with the body active. Direction defaults to `0` (either edge). All five LRM arguments are **honoured**: `enable` gates the site (a disabled one registers nothing, so it cannot fire), and `time_tol`/`expr_tol` drive a bracketing retry — a step revealing a crossing that overshoots the tolerance is rejected and re-taken to land inside it, through the same retry path the LTE controller uses. Convergence reaches ~70 ulps of the timescale (an attosecond at t≈83µs); a request below f64 spacing is reported via `Waveform::unresolved_events`, never quietly missed. A *compound* trigger (`initial_step or cross(...)`) is still refused, as is a `cross` whose monitored expression reads a local variable | Zero-crossing event detector (LRM §5.7ish) | Neither path currently evaluates its arguments | Analog-block only (event control) | No C analogue (continuous zero-crossing detection needs the solver's own state) |
...
| `final_step` | Reserved, no grammar production as a bare word outside `@()`; its real usage, `@(final_step)`, is **implemented** (2026-09-07, v0.9.8): the parser desugars it to `if (final_step()) …`, elaboration lowers that to `va_ir::Builtin::FinalStep`, and codegen reads `va_abi::AnalysisCtx::is_final_step`. `true` in every static analysis (one solve point is both first and last, so a swept DC runs the body at each point — the same rule `initial_step` already follows) and, in transient, only at the last accepted timepoint, which the integrator solves a second time so the body's effect is in the point recorded. An `or` list of step-scoped events (`initial_step or final_step`) composes into a `||` chain (v0.9.9); every other compound, and the analysis-filter form `final_step("tran")`, is **refused** rather than discarded — a discarded filter runs the body in the analyses the model excluded, which is wrong in every analysis. **Stated asymmetry with `initial_step`:** only `final_step`'s timepoint is re-solved, so only its body can change a recorded point (see `initial_step`'s row) | Global analog event: fires once at analysis end | N/A | Analog-block only (event control) | No C analogue (closest: an `atexit()` hook) |
...
| `initial_step` | Reserved, no grammar production as a bare word outside `@()`; its real usage, `@(initial_step)`, is **implemented** (2026-08-06): desugared to `if (initial_step()) …` and lowered to `va_ir::Builtin::InitialStep`, which reads `va_abi::AnalysisCtx::is_initial_step`. An `or` list with `final_step` composes into a `||` chain (v0.9.9); the optional `(analysis_list)` filter is **refused** rather than silently dropped, as is every other compound. **Stated limitation, older than the refusal and easy to miss:** `is_initial_step` is set on exactly one transient evaluation — the seed assemble at `tstart`, which establishes state history and is never *solved*, `x0` being the caller's initial condition. So an `@(initial_step)` body influences committed state (the reason the flag exists) but changes no recorded point. `final_step`, whose timepoint *is* re-solved, does | Global analog event: fires once at analysis start | N/A | Analog-block only (event control) | No C analogue (closest: a one-time setup routine) |
...
| `laplace_nd` | **Implemented in AC** (Tier C, 2026-08-07): `H(jω)` is evaluated per frequency point and stamped as `G = Re(H)`, `C = Im(H)/ω`, which the assembled `G + jω·C` reconstitutes exactly. **Implemented in DC and transient as of v0.9.16 (2026-09-11)** as a state-space realization on auxiliary unknowns (`va_codegen::lower::LaplaceStates`): a denominator of degree `m` claims `m` state unknowns in controllable canonical form, each with its own `ddt`-shaped row, so the existing integrator (companion model, LTE, order) applies unchanged; the output is a linear combination of the states plus a feedthrough when `deg N = deg D`. At DC the same rows give `H(0)`. Root forms are expanded to coefficients for this path only (AC keeps the product form). Refused at build: a pole at the origin (`d_0 = 0`, use `idt`), an improper numerator (`deg N > deg D`, use `ddt`), roots not in conjugate pairs. Gated by `circuits/laplace_step.net` vs QSPICE (5.1e-6) and closed-form step responses of first- and second-order filters. Noise still evaluates `H(0)`, a stated limitation. Parses as a call (`laplace_nd(in, num[, den])`); elaboration has no builtin → `unknown function` | Laplace-domain transfer-function filter, numerator/denominator coefficient form | `H(jω)` per frequency point in AC; an ODE on state unknowns in DC/transient; `H(0)` in noise | Analog-block only, signal-flow filter | No C analogue (a continuous-time transfer function) |
| `laplace_np` | **Implemented in AC** (Tier C, 2026-08-07): `H(jω)` is evaluated per frequency point and stamped as `G = Re(H)`, `C = Im(H)/ω`, which the assembled `G + jω·C` reconstitutes exactly. **Implemented in DC and transient as of v0.9.16 (2026-09-11)** as a state-space realization on auxiliary unknowns (`va_codegen::lower::LaplaceStates`): a denominator of degree `m` claims `m` state unknowns in controllable canonical form, each with its own `ddt`-shaped row, so the existing integrator (companion model, LTE, order) applies unchanged; the output is a linear combination of the states plus a feedthrough when `deg N = deg D`. At DC the same rows give `H(0)`. Root forms are expanded to coefficients for this path only (AC keeps the product form). Refused at build: a pole at the origin (`d_0 = 0`, use `idt`), an improper numerator (`deg N > deg D`, use `ddt`), roots not in conjugate pairs. Gated by `circuits/laplace_step.net` vs QSPICE (5.1e-6) and closed-form step responses of first- and second-order filters. Noise still evaluates `H(0)`, a stated limitation. Same family as `laplace_nd`, pole/zero form | Laplace-domain filter, pole/zero form | `H(jω)` per frequency point in AC; an ODE on state unknowns in DC/transient; `H(0)` in noise | Analog-block only | No C analogue |
| `laplace_zd` | **Implemented in AC** (Tier C, 2026-08-07): `H(jω)` is evaluated per frequency point and stamped as `G = Re(H)`, `C = Im(H)/ω`, which the assembled `G + jω·C` reconstitutes exactly. **Implemented in DC and transient as of v0.9.16 (2026-09-11)** as a state-space realization on auxiliary unknowns (`va_codegen::lower::LaplaceStates`): a denominator of degree `m` claims `m` state unknowns in controllable canonical form, each with its own `ddt`-shaped row, so the existing integrator (companion model, LTE, order) applies unchanged; the output is a linear combination of the states plus a feedthrough when `deg N = deg D`. At DC the same rows give `H(0)`. Root forms are expanded to coefficients for this path only (AC keeps the product form). Refused at build: a pole at the origin (`d_0 = 0`, use `idt`), an improper numerator (`deg N > deg D`, use `ddt`), roots not in conjugate pairs. Gated by `circuits/laplace_step.net` vs QSPICE (5.1e-6) and closed-form step responses of first- and second-order filters. Noise still evaluates `H(0)`, a stated limitation. Same family, Z-domain numerator/denominator form | Z-domain (discrete) filter | `H(jω)` per frequency point in AC; an ODE on state unknowns in DC/transient; `H(0)` in noise | Analog-block only | Closest: a digital IIR filter's difference equation, but expressed declaratively |
| `laplace_zp` | **Implemented in AC** (Tier C, 2026-08-07): `H(jω)` is evaluated per frequency point and stamped as `G = Re(H)`, `C = Im(H)/ω`, which the assembled `G + jω·C` reconstitutes exactly. **Implemented in DC and transient as of v0.9.16 (2026-09-11)** as a state-space realization on auxiliary unknowns (`va_codegen::lower::LaplaceStates`): a denominator of degree `m` claims `m` state unknowns in controllable canonical form, each with its own `ddt`-shaped row, so the existing integrator (companion model, LTE, order) applies unchanged; the output is a linear combination of the states plus a feedthrough when `deg N = deg D`. At DC the same rows give `H(0)`. Root forms are expanded to coefficients for this path only (AC keeps the product form). Refused at build: a pole at the origin (`d_0 = 0`, use `idt`), an improper numerator (`deg N > deg D`, use `ddt`), roots not in conjugate pairs. Gated by `circuits/laplace_step.net` vs QSPICE (5.1e-6) and closed-form step responses of first- and second-order filters. Noise still evaluates `H(0)`, a stated limitation. Same family, Z-domain pole/zero form | Z-domain (discrete) filter | `H(jω)` per frequency point in AC; an ODE on state unknowns in DC/transient; `H(0)` in noise | Analog-block only | Same as `laplace_zd` |
...
| `last_crossing` | Parses as a call (`last_crossing(expr, dir)`); elaboration has no builtin → `unknown function` | Returns the simulation time of the last zero-crossing of `expr` | `H(jω)` per frequency point in AC; an ODE on state unknowns in DC/transient; `H(0)` in noise | Analog-block only | No C analogue |
...
| `$table_model` | **Implemented, one dimension, file source** (2026-09-22, v1.3.0) — LRM §9.21. Lowered to `Builtin::TableModel`. The data file is read **at elaboration**, which is what the LRM itself describes (§9.21.1: the source's state "is captured on the first call") and what keeps `ModelInstance::load` a pure function of `(x, ctx, committed state)` — a lookup that touched the filesystem per evaluation would be re-run by every Newton iteration and every rejected timestep. The table is folded into the call's arguments as ordinary `Const`s, the same trade `noise_table` makes, so every arena walk and clone keeps working unchanged (Interface α change — see `docs/interfaces.md`). **Refused, not approximated:** a second lookup expression (N-D), the array data source, `2`/`3` spline interpolation, `I` ignore-column, `E` error-extrapolation, and the `;N` dependent-column selector. Each is a `Refusal` naming what to write instead; a missing or malformed file stays a plain elaboration error, since that is a broken input rather than a declined construct | `$table_model(x, "file" [, "control"])`; the data source must be a string literal. File format: one `x y` pair per line, whitespace- or comma-separated, `*`/`#`/`//` comments and blank lines skipped. Rows need not be sorted — elaboration sorts them, drops exact duplicates and **errors** on two rows giving one `x` two different `y` (LRM §9.21.1). At least two distinct points required | Piecewise-linear in `(x, y)`, or closest-point with `D`. Derivative is the segment slope, finite-difference gated per §5 (`table_model_derivative_matches_finite_difference`); at a knot the function has a kink and the right-hand segment's slope is taken, the subgradient convention `abs` uses at zero. `D` is a step, so its slope is `0`. Extrapolation per end: `C` clamps to the endpoint (slope `0`), `L` continues the end segment's slope. **Defaults are the LRM's**: linear interpolation and *linear* extrapolation at both ends (`"1LL"`), not the constant extrapolation a reader might assume — an omitted control string is `122` in the packed code | Analog-block only; an ordinary expression, so it may appear anywhere a value may, including inside a contribution and driven by a probe. Bias-independent when its argument is, in which case v1.2.0's setup/eval split hoists it out of the Newton loop entirely | A lookup table with interpolation — C's nearest equivalent is a hand-written `bsearch` plus lerp, except this one has to supply its own derivative or the solver converges to the wrong answer |
...
| `timer` | Parses as a call (`timer(start[, period])`) if written bare; its real usage, `@(timer(start, period))`, is **implemented** (2026-09-06, v0.9.4): elaboration registers a `va_ir::EventSite::Timer` and desugars the body to `if (EventFired(k)) …`, the model re-registers its next occurrence at each accepted timepoint via `EventSink::timer`, and the integrator lands on it and fires. Period defaults to `0` (fire once); `enable` is honoured. `time_tol` needs nothing: the LRM asks for a timepoint *within* it and this engine lands exactly, so any non-negative tolerance is already met — which is why, unlike `cross`'s, it raises no warning. An occurrence on the run's initial timepoint is not delivered, and a compound trigger is still refused | Fires at a specified absolute/periodic simulation time | Evaluated per accepted timepoint | Analog-block only (event control) | Closest to a POSIX interval timer/`setitimer` |
...
| `zi_nd` | **Implemented (v0.9.20, 2026-09-12; `docs/proposals/z-domain-filters.md`)** in all three analyses, lowered to `Builtin::ZiNd` with `[value, T, tt, t0, Const(num_len), num…, den…]`: DC and noise evaluate the steady-state gain `H(1)`; AC evaluates `H(e^{jωT})` per point through the same complex-gain stamp as `laplace_*` (no zero-order-hold factor; aliases above `π/T` as the mathematics does); transient runs the difference equation `d_0·y_k = Σ n_i·x_{k−i} − Σ_{j≥1} d_j·y_{k−j}` on the state channel, sampling the input only at `t0 + k·T` — instants the instance asks the integrator to land on via a breakpoint — and ramping the output over the transition time (omitted → `` `default_transition ``; zero → the negligible `tstep/1000` `transition` also uses). The output never depends on the current Newton iterate, so its Jacobian contribution is exactly zero. Roots follow LRM 4.5.12 (`(1 − r·z^-1)`; the origin is the factor `z`); refused at build: a lone complex root, `d_0 = 0`, more numerator than denominator origin roots (an advance), `T ≤ 0`, `tt < 0`. History: folded to `H(1)` in every analysis until v0.9.17, refused v0.9.17–0.9.19. Gated against closed forms (sample-and-hold staircase, one-pole IIR `1 − a^k`, a conjugate-pair recurrence, `H(e^{jωT})` across a decade) — QSPICE has no LRM Z-filter to compare to. Parses as a call (`zi_nd(in, num, den, T[, tt[, t0]])`) | Z-domain (sampled-data) IIR filter, numerator/denominator coefficients | `H(1)` in DC/noise; `H(e^{jωT})` in AC; a sampled difference equation on the state channel in transient | Analog-block only, signal-flow filter | Closest: a digital IIR filter's difference equation, expressed declaratively |
| `zi_np` | **Implemented (v0.9.20, 2026-09-12; `docs/proposals/z-domain-filters.md`)** in all three analyses, lowered to `Builtin::ZiNp` with `[value, T, tt, t0, Const(num_len), num…, den…]`: DC and noise evaluate the steady-state gain `H(1)`; AC evaluates `H(e^{jωT})` per point through the same complex-gain stamp as `laplace_*` (no zero-order-hold factor; aliases above `π/T` as the mathematics does); transient runs the difference equation `d_0·y_k = Σ n_i·x_{k−i} − Σ_{j≥1} d_j·y_{k−j}` on the state channel, sampling the input only at `t0 + k·T` — instants the instance asks the integrator to land on via a breakpoint — and ramping the output over the transition time (omitted → `` `default_transition ``; zero → the negligible `tstep/1000` `transition` also uses). The output never depends on the current Newton iterate, so its Jacobian contribution is exactly zero. Roots follow LRM 4.5.12 (`(1 − r·z^-1)`; the origin is the factor `z`); refused at build: a lone complex root, `d_0 = 0`, more numerator than denominator origin roots (an advance), `T ≤ 0`, `tt < 0`. History: folded to `H(1)` in every analysis until v0.9.17, refused v0.9.17–0.9.19. Gated against closed forms (sample-and-hold staircase, one-pole IIR `1 − a^k`, a conjugate-pair recurrence, `H(e^{jωT})` across a decade) — QSPICE has no LRM Z-filter to compare to. Parses as a call (`zi_np(in, num, den, T[, tt[, t0]])`) | Z-domain (sampled-data) IIR filter, numerator coefficients over a pole array | `H(1)` in DC/noise; `H(e^{jωT})` in AC; a sampled difference equation on the state channel in transient | Analog-block only | Same as `zi_nd` |
| `zi_zd` | **Implemented (v0.9.20, 2026-09-12; `docs/proposals/z-domain-filters.md`)** in all three analyses, lowered to `Builtin::ZiZd` with `[value, T, tt, t0, Const(num_len), num…, den…]`: DC and noise evaluate the steady-state gain `H(1)`; AC evaluates `H(e^{jωT})` per point through the same complex-gain stamp as `laplace_*` (no zero-order-hold factor; aliases above `π/T` as the mathematics does); transient runs the difference equation `d_0·y_k = Σ n_i·x_{k−i} − Σ_{j≥1} d_j·y_{k−j}` on the state channel, sampling the input only at `t0 + k·T` — instants the instance asks the integrator to land on via a breakpoint — and ramping the output over the transition time (omitted → `` `default_transition ``; zero → the negligible `tstep/1000` `transition` also uses). The output never depends on the current Newton iterate, so its Jacobian contribution is exactly zero. Roots follow LRM 4.5.12 (`(1 − r·z^-1)`; the origin is the factor `z`); refused at build: a lone complex root, `d_0 = 0`, more numerator than denominator origin roots (an advance), `T ≤ 0`, `tt < 0`. History: folded to `H(1)` in every analysis until v0.9.17, refused v0.9.17–0.9.19. Gated against closed forms (sample-and-hold staircase, one-pole IIR `1 − a^k`, a conjugate-pair recurrence, `H(e^{jωT})` across a decade) — QSPICE has no LRM Z-filter to compare to. Parses as a call (`zi_zd(in, num, den, T[, tt[, t0]])`) | Z-domain (sampled-data) IIR filter, a zero array over denominator coefficients | `H(1)` in DC/noise; `H(e^{jωT})` in AC; a sampled difference equation on the state channel in transient | Analog-block only | Same as `zi_nd` |
| `zi_zp` | **Implemented (v0.9.20, 2026-09-12; `docs/proposals/z-domain-filters.md`)** in all three analyses, lowered to `Builtin::ZiZp` with `[value, T, tt, t0, Const(num_len), num…, den…]`: DC and noise evaluate the steady-state gain `H(1)`; AC evaluates `H(e^{jωT})` per point through the same complex-gain stamp as `laplace_*` (no zero-order-hold factor; aliases above `π/T` as the mathematics does); transient runs the difference equation `d_0·y_k = Σ n_i·x_{k−i} − Σ_{j≥1} d_j·y_{k−j}` on the state channel, sampling the input only at `t0 + k·T` — instants the instance asks the integrator to land on via a breakpoint — and ramping the output over the transition time (omitted → `` `default_transition ``; zero → the negligible `tstep/1000` `transition` also uses). The output never depends on the current Newton iterate, so its Jacobian contribution is exactly zero. Roots follow LRM 4.5.12 (`(1 − r·z^-1)`; the origin is the factor `z`); refused at build: a lone complex root, `d_0 = 0`, more numerator than denominator origin roots (an advance), `T ≤ 0`, `tt < 0`. History: folded to `H(1)` in every analysis until v0.9.17, refused v0.9.17–0.9.19. Gated against closed forms (sample-and-hold staircase, one-pole IIR `1 − a^k`, a conjugate-pair recurrence, `H(e^{jωT})` across a decade) — QSPICE has no LRM Z-filter to compare to. Parses as a call (`zi_zp(in, num, den, T[, tt[, t0]])`) | Z-domain (sampled-data) IIR filter, zero array over pole array | `H(1)` in DC/noise; `H(e^{jωT})` in AC; a sampled difference equation on the state channel in transient | Analog-block only | Same as `zi_nd` |
