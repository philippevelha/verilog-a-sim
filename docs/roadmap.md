...
## Road to 2.0 — ratified 2026-09-26

`release.txt`'s versioning rule makes 2.0 a milestone, not a count: 1.0 was reached when its
"Road to 1.0" list closed, and 2.0 is reached when this one does. Proposed the same day and
**ratified by the user: items 1, 2 and 3 are the blockers.** The theme is **trusted at scale** —
1.0 meant "the language front end and every analysis validated on small circuits"; 2.0 means the
same trust at ISCAS'85 scale. Each item states the gate that closes it; an item closes only when
its gate is green, recorded here and in a release entry.

1. **The ISCAS'85 suite at transistor level, every circuit's `.op` checked against its logic.**
   c17 and c432 are done (`check_c17.py truth`, `check_iscas.py`); c499–c7552 are to be generated
   by `gen_iscas.py` from the same cells (composing any gate the c7552 library lacks — the rule of
   `circuits/benchmark/iscas85/README.md`). **Gate:** every circuit solves and every gate output
   matches the `.bench` logic for a set of input vectors. Evidence it is reachable: c432's `.op`
   takes 11.5 s on 8 threads; c7552 has ~22× c432's gates (3 512 against 160), and evaluation, not
   the solve, is what scales with it. **Risk, stated:** the 320-stage PSP103 chain (10 566
   unknowns) still fails, by a mechanism not diagnosed (`dc.rs`, `RESCUE_DAMPING_HALVINGS`) — the
   first thing to understand before scaling further.
2. **A transient of c432 checked against its logic,** the way c17's is (`check_c17.py tran`).
   **Gate:** every settled sample of every output matches the logic of the inputs at that time.
   Needs evaluation to get cheaper (`docs/proposals/transient-solve.md` (C)–(E)): 1 ns of c432
   takes ~15 min on 8 threads today (878 s at 1.26.0), and a check needs several input vectors'
   worth of settled time.
3. **Reproducibility measured across platforms, not only threads.** 1.16.1–1.18.0 made a run
   bit-identical at every thread count on one machine, and `libm` (1.17.0) took the platform maths
   library out of the answer — but "the same bits on Linux, macOS and Windows" is a claim, not yet
   a measurement. **Gate:** CI runs the deck set on all three and a `deck-diff` of their outputs
   is identical (or every difference is explained and bounded, in writing).

**Not blockers for 2.0** (kept in `docs/future_development.md`, to be done when a user needs
them): a machine-readable results file (its item 2), `absdelay` in transient (item 3), Interface β
changes such as a seeded RNG for `$rdist_*` and a sensitivity channel (items 4–5, each a §6
event on its own schedule), GPU execution, PSS/harmonic balance, a Python binding.
