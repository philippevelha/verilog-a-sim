...
## Reproducible, parallel, and 7× faster on c432 (2026-09-26, 1.16.1 → 1.26.0)

One day, eleven releases, each gated by `deck-diff --all` against the release before it and by
`validate`, with every answer that moved measured and recorded in `release.txt`:

| release | what | c432 `.op` |
|---|---|---:|
| 1.16.0 | (start of the day, 1 thread) | 87.5 s |
| 1.16.1 | `faer` pinned sequential — the same deck gave different last digits on machines with different core counts | — |
| 1.17.0 | every simulated transcendental through the pure-Rust `libm` (MinGW's x87 maths was per-thread); a clippy rule keeps it | — |
| 1.18.0 | device instances evaluated in parallel, recorded and replayed in instance order: **bit-identical to serial at every thread count** (§6 change: `ModelInstance: Send + Sync`) | 43.6 s (8 thr) |
| 1.21.0 | a block-triangular DC solve: by value, a logic circuit's Jacobian is ~12 600 blocks of ≤ 34 unknowns; `faer` kept as the fallback | 28.0 s |
| 1.23.0 | the DC rescue runs the node-step-capped `gmin` ladder first (the plain one spent 265 iterations failing) | 15.2 s |
| 1.24.0 | the rescue's `gmin` ramp adapts its step (doubles after fast stages, halves and retries after a failure) | **11.5 s** |
| 1.26.0 | transient steps use the block solve, a large block through `faer` alone: c432's 1 ns transient 1 300 → 878 s | — |

Decision documents, each with the measurement that decided it: `docs/proposals/`
`parallel-assembly.md`, `btf-solver.md`, `dc-rescue.md`, `adaptive-gmin.md`, `transient-solve.md`,
`transient-block-solve.md`. Tools added to measure before building: `xtask btf` (block structure,
block-solve prototypes, Newton tracking), `xtask ladder` (rescue schedules), `deck-diff
--env-old/--env-new`. A c432-size transient deck (`c432_tran.net`, 1 ns) now exists.

Findings worth keeping beyond the numbers:

- **A test asserted a one-ulp accident.** `gmin_stepping_converges_a_circuit_plain_newton_cannot`
  claimed failure at any iteration budget; MinGW's `ln` made that true at exactly one cell of a
  5–40 diode × 5–160 V sweep. Restated to the property that holds (failure within the default
  budget).
- **Fewer equal `gmin` steps is not "faster": it is "fails".** At 10/15/20 stages three PSP103
  chains were not solved; the step had to adapt, not start larger.
- **Stall detection has no clean signal**: converging stages also go 40–56 iterations without a
  10× improvement.
- **Pointwise comparison of a trapezoidal transient across different step sequences lies**: a
  source current alternating ±25% per step reads as a 44% "difference" that is not there.

What it leaves: device evaluation is now most of every large run — c432's `.op` is 228 Newton
iterations of 910 PSP103 evaluations each, with the solve down to ~2 ms of each; c17's transient
is 85% evaluation and scales only 2.3× on 8 threads at ~30 instances; c432's transient was 52%
evaluation before 1.26.0 cut its solve. `docs/proposals/transient-solve.md` options (C)–(E) are
the open levers.

## Toward 2.0 — proposed, not ratified (2026-09-26)

`release.txt`'s versioning rule makes 2.0 a milestone, not a count: 1.0 was reached when its
"Road to 1.0" list closed. Nothing below is decided; it is the list proposed for the user to
ratify, cut, or reorder — each item with the evidence that motivates it and the gate that would
close it. `docs/future_development.md` holds the longer list of what the simulator *could* do;
these are the items proposed as what 2.0 *requires*.

**The theme proposed for 2.0: trusted at scale.** 1.0 meant "the language front end and every
analysis validated on small circuits". Today's work made c432 (15 416 unknowns) solve in seconds
and its transient in minutes. 2.0 would mean the same trust at ISCAS'85 scale that 1.0 gave the
zoo.

1. **The ISCAS'85 suite at transistor level, every circuit's `.op` checked against its logic.**
   c17 and c432 are done (`check_iscas.py`); c499–c7552 are generated by `gen_iscas.py` from the
   same cells. Gate: every circuit solves and every gate output matches the `.bench` logic for
   a set of vectors. Evidence it is reachable: c432 `.op` 11.5 s on 8 threads today; c7552 has
   ~22× c432's gates (3 512 against 160), and evaluation, not the solve, is what scales with it. **Risk, stated:** the 320-stage PSP103 chain (10 566 unknowns) still
   fails, by a mechanism not diagnosed (`dc.rs`, `RESCUE_DAMPING_HALVINGS`) — the first thing to
   understand before scaling further.
2. **A transient of c432 checked against its logic,** the way c17's is (`check_c17.py tran`).
   Gate: every settled sample of every output matches. Needs evaluation to get cheaper
   (`transient-solve.md` (C)–(E)): 1 ns takes 15 min today.
3. **Reproducibility checked across platforms, not only threads.** 1.16.1–1.18.0 made a run
   bit-identical at every thread count on one machine, and `libm` removed the platform maths
   library from the answer — but "the same bits on Linux, macOS and Windows" is a claim, not yet
   a measurement. Gate: CI runs the deck set on all three and a `deck-diff` of their outputs is
   identical.
4. **A machine-readable results file** (`future_development.md` item 2). Gate: every analysis
   writes it; `va-harness` reads its own golden comparisons from it rather than parsing the
   printed report.
5. **The last transient refusal in the analog-operator family: `absdelay`**
   (`future_development.md` item 3; `docs/proposals/absdelay.md` stage 2). Gate: the photonic
   waveguide decks' group delay in transient, validated.
6. **Interface changes batched at the major version.** Items the future list needs that change
   Interface β — a seeded RNG in the analysis context for `$rdist_*` and Monte Carlo (item 5),
   a parameter-sensitivity channel for fitting (item 4) — are §6 events. 2.0 is the natural
   place to ratify them together rather than one per minor release.

Proposed as **not** required for 2.0: GPU execution, PSS/harmonic balance, a Python binding —
each valuable, none needed for "trusted at scale".

**Decide first:** whether "trusted at scale" is the 2.0 theme at all, and which of 1–6 are
blockers rather than wishes.
