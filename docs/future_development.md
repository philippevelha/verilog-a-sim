...
>
> **2026-09-26 — the solve made reproducible and then fast, 1.16.1–1.26.0.** `faer` pinned
> sequential (its default parallel LU rounded by core count); a block-triangular front end
> (`va_core::btf`) for DC (1.21.0: c432's solve 40 → ~2 ms) and for transient steps, with the
> one large block through `faer` alone (1.26.0: c432's 1 ns transient 1 300 → 878 s). The
> 10 566-unknown chain is still undiagnosed. `docs/proposals/btf-solver.md`,
> `transient-block-solve.md`; roadmap, 2026-09-26.
...
> **2026-09-26 — (2) shipped in 1.18.0** (`va_core::par`): each instance evaluated into its own
> stamp recording, replayed in instance order, so the result is **bit-identical to serial at every
> thread count** — the "fixed merge order" above, taken literally. Parallel only when measured
> evaluation work per assembly is ≥ 200 µs. It needed `libm` for every transcendental (1.17.0:
> MinGW's maths gave different bits on different threads) and `ModelInstance: Send + Sync` (a §6
> change). c432 `.op` 1.84× on 8 threads at the time; c17's transient only 2.3× (~30 instances).
> `docs/proposals/parallel-assembly.md`.
